...
- **Per-tenant DBs**: `rwadsolu_tenant_{uuid}` (prefix in `config/tenancy.php`). Provisioning is **queued** (background): `CreateTenant.php` + the API controller emit `TenantCreated`, whose pipeline runs `CreateDatabase` → `MigrateDatabase` via `JobPipeline->shouldBeQueued(true)` (see `app/Providers/TenancyServiceProvider.php`). `SeedTenantDatabase` (queued, `ShouldQueue`) is dispatched after commit from `CreateTenant.php` and the API controller. Order is FIFO, so the seed always runs after migration. Requires a queue worker (`php artisan queue:work`); with `QUEUE_CONNECTION=sync` (dev default) everything still runs inline.
...
- Deploy workflows delete `public/css/app/{custom-stylesheet,subscriptions-page,whatsapp-ui,messaging-health-dashboard,meta-integrations-reset,crm-custom-stylesheet,accounting-reports}.css` and back up `public/.htaccess` before `git checkout`/`reset --hard` to avoid merge conflicts with generated/server files.
