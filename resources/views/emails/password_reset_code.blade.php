...
<head>
    ...
    <title>{{ __('auth.password_reset_code_subject') }}</title>
</head>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #eee;">
        <tr>
            <td style="padding:24px;">
                <h2 style="margin:0 0 12px 0;">{{ __('auth.password_reset_code_subject') }}</h2>
                <p style="margin:0 0 16px 0;">{{ __('auth.use_code_to_reset_password') }}</p>
