@php
    /** @var \App\Models\Tenant\WhatsAppApiRequest $request */
...
<div class="wa-log-detail">
    <section class="wa-log-card">
        ...
        <dl class="wa-log-grid">
            <div>
                <dt>{{ __('dashboard.whatsapp_api_operation') }}</dt>
                <dd>{{ $request->operation?->label() ?? '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_api_outcome') }}</dt>
                <dd>{{ $request->outcome?->label() ?? '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_api_status_label') }}</dt>
                <dd>{{ $request->status_label }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_customer_phone') }}</dt>
                <dd>{{ $request->recipient_phone ?: '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_number') }}</dt>
                <dd>{{ $request->whatsappNumber?->display_phone_number ?: '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_http_status') }}</dt>
                <dd>{{ $request->http_status ?: '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_api_error_code') }}</dt>
                <dd>{{ $request->api_error_code ?: '—' }}</dd>
            </div>
            <div>
                <dt>{{ __('dashboard.whatsapp_api_duration') }}</dt>
    ...
    @if ($showTechnical)
        @if (is_array($request->request_payload))
            <section class="wa-log-card">
                <h3 class="wa-log-card__title">{{ __('dashboard.whatsapp_api_request_payload') }}</h3>
        ...
        @if (is_array($request->response_body))
            <section class="wa-log-card">
                <h3 class="wa-log-card__title">{{ __('dashboard.whatsapp_api_response_body') }}</h3>
