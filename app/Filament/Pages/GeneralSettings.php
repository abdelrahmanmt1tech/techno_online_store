...
class GeneralSettings extends Page
{
    ...
    {
        return Auth::user()->can('general-settings.view');
    }
                ...
                Form::make([
                    Section::make(__('dashboard.general_settings_section'))
                        ...
                        ->schema([
                            ...
                            TextInput::make('courses_link')
                                ->label(__('dashboard.general_settings.courses_link'))
                                ->url()
                                ->maxLength(255)
                                ->columnSpan(1),
                        ])
                    ...
                    Section::make(__('dashboard.terms_settings_section'))
                        ->columns(2)
                        ->schema([
                            RichEditor::make('terms_and_conditions_ar')
                                ->label(__('dashboard.terms_settings.terms_ar'))
                                ->columnSpan(1),

                            RichEditor::make('terms_and_conditions_en')
                                ->label(__('dashboard.terms_settings.terms_en'))
                                ->columnSpan(1),
                        ])
                        ->icon(Heroicon::DocumentText)
                        ->columnSpanFull(),

                    Section::make(__('dashboard.privacy_settings_section'))
                        ->columns(2)
                        ->schema([
                            RichEditor::make('privacy_policy_ar')
                                ->label(__('dashboard.privacy_settings.privacy_ar'))
                                ->columnSpan(1),

                            RichEditor::make('privacy_policy_en')
                                ->label(__('dashboard.privacy_settings.privacy_en'))
                                ->columnSpan(1),
                        ])
                    ...
                    ->footer([
                        Actions::make([
                            Action::make('save')
                                ...
                                ->keyBindings(['mod+s'])
                                ->visible(fn () => Auth::user()->can('general-settings.update')),
                        ]),
                    ]),
            ])
            ->statePath('data');
    ...
    public function save(): void
    {
        ...
        $richEditorKeys = ['terms_and_conditions_ar', 'terms_and_conditions_en', 'privacy_policy_ar', 'privacy_policy_en'];
        ...
        foreach ($data as $key => $value) {
            ...
            } elseif (in_array($key, $tagInputKeys)) {
                Setting::updateOrCreate(
                    ['key' => $key],
    ...
    public function getRecord()
    {
        ...
        foreach ($richEditorKeys as $key) {
            $settings[$key] = Setting::where('key', $key)->value('string_value') ?? '';
        }
